...
const navItems = [
    ...
    { name: 'Portfolio', route: 'portfolio', icon: 'folder_shared' }, 
    { name: 'Leaderboard', route: 'leaderboard', icon: 'leaderboard' },
    
];

const pageTitle = computed(() => {
    if (route().current('dashboard')) return 'Dashboard Overview';
    if (route().current('classroom.my-courses')) return 'My Course';
    if (route().current('profile.edit')) return 'Account Settings';
...
<template>
    <div class="bg-background-light font-display text-text-main antialiased selection:bg-primary selection:text-black flex h-screen overflow-hidden">
        ...
        <aside :class="[
            'fixed inset-y-0 left-0 z-50 w-64 bg-sidebar-bg text-white flex flex-col h-full shrink-0 transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0',
            isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'
        ]">
            
            <div class="h-20 flex items-center justify-between px-6 border-b border-white/10 shrink-0">
                <div class="flex items-center gap-3 cursor-pointer" @click="router.visit(route('dashboard'))">
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-3 custom-scrollbar">
                ...
                >
                    <span class="material-symbols-outlined" :class="{ 'text-primary': isActive(item.route) }">
                        {{ item.icon }}
                    </span>
                ...
                <Link :href="route().has('profile.edit') ? route('profile.edit') : '#'" :class="[
            ...
            <div class="p-4 border-t border-white/10 shrink-0">
                <button @click="logout" class="flex items-center gap-3 w-full px-3 py-2 text-gray-400 hover:text-red-400 transition-colors group">
        ...
        <main class="flex-1 flex flex-col h-full relative overflow-hidden bg-gray-50 min-w-0">
            ...
            <header class="h-16 sm:h-20 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-8 flex-shrink-0 z-10">
                ...
                <div class="flex items-center gap-4 ml-4 flex-shrink-0">
                    ...
                    <div class="hidden md:flex items-center gap-2 bg-gray-50 rounded-full p-1 pr-4 border border-gray-200 mr-2">
                        <div class="flex items-center gap-1 bg-white px-3 py-1.5 rounded-full shadow-sm border border-gray-100">
                            <span class="material-symbols-outlined text-primary text-sm">bolt</span>
                            <span class="text-xs font-bold text-gray-900">{{ user.xp }} XP</span>
                        </div>
                        <div class="flex items-center gap-1 text-gray-600">
                            <span class="material-symbols-outlined text-blue-500 text-sm">monetization_on</span>
                            <span class="text-xs font-bold">{{ user.credits }} Cr</span>
                        </div>
                    </div>

                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-semibold text-gray-800">{{ user.name }}</p>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">{{ user.class }} • NIS: {{ user.nis }}</p>
                    </div>
                    
                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gray-200 overflow-hidden border border-gray-100 flex-shrink-0">
                        <img 
                            :src="user.avatar || `https://ui-avatars.com/api/?name=${user.name}&background=ffde21&color=000`" 
                            alt="User" 
