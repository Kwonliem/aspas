...
const navItems = [
    { name: 'Dashboard', route: 'teacher.dashboard', icon: 'dashboard' },
    { name: 'My Courses', route: 'teacher.courses', icon: 'menu_book' },
    { name: 'Pending Submissions', route: 'teacher.submissions.index', icon: 'assignment_turned_in' },
    { name: 'Weekly Challenges', route: 'teacher.challenges', icon: 'emoji_events' }, 
];

const pageTitle = computed(() => {
    if (route().current('teacher.dashboard')) return 'Dashboard Overview';
    if (route().current('teacher.courses*')) return 'My Courses';
    if (route().current('teacher.submissions.index')) return 'Pending Submissions';
    if (route().current('teacher.challenges*')) return 'Weekly Challenges'; 
    if (route().current('teacher.settings')) return 'Account Settings';
    ...
    <div class="bg-background-light font-display text-text-main antialiased selection:bg-primary selection:text-black flex h-screen overflow-hidden">
        ...
        <aside :class="[
            ...
            <div class="h-20 flex items-center justify-between px-6 border-b border-white/10 shrink-0">
                <div class="flex items-center gap-3 cursor-pointer" @click="router.visit(route('teacher.dashboard'))">
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-3 custom-scrollbar">
                ...
                <Link :href="route('teacher.settings')" :class="[
                    'flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 group',
                    route().current('teacher.settings')
                        ? 'bg-gradient-to-r from-primary/20 to-transparent text-primary border-l-4 border-primary'
                        : 'text-gray-400 hover:text-white hover:bg-white/5'
                ]">
            ...
            <header class="h-16 sm:h-20 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-8 flex-shrink-0 z-10">
                ...
                <div class="flex items-center gap-4 ml-4 flex-shrink-0">
                    <div class="text-right hidden sm:block">
                        ...
                        <p class="text-xs text-gray-500 uppercase tracking-wider">{{ user.subject }}</p>
                    </div>
